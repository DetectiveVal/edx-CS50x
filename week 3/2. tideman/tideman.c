...
bool paircheck(int start, int end);
...
{
    for(int i = 0; i < pair_count; i++){
        if(!paircheck(pairs[i].winner,pairs[i].loser)){
...
bool paircheck(int start, int end){
    ...
    for(int i = 0; i < candidate_count; i++){
        if(locked[end][i] && paircheck(start,i)){
            return true;
        }
    }
    return false;
